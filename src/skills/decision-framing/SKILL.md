...
5. One four-choice reverse question that helps the user clarify their own preference.

## Reverse Question Rule

When the user is still vague, ask one multiple-choice question with exactly four options. The fourth option should be `직접 입력`.
...
Use these questions to help the user articulate a preference, not to force a product recommendation.
...
For structured AX-style reverse questions, use:
