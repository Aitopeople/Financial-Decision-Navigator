...
이 플러그인을 로컬 personal marketplace에 등록한 뒤 설치합니다.

```bash
codex plugin add financial-decision-navigator@personal
```

처음 설치하는 경우에는 플러그인 폴더가 아래 위치에 있어야 합니다.

```text
~/plugins/financial-decision-navigator
```

그리고 `~/.agents/plugins/marketplace.json`의 personal marketplace에 `financial-decision-navigator` 항목이 등록되어 있어야 합니다.
...
설치 후 Codex에서 플러그인을 선택한 뒤 자연어로 질문하면 됩니다.
